...
export const PayoutConfirmScreen: React.FC<NavScreenProps> = ({ params, go }) => {
  ...
  return (
    <div className="p-4 space-y-4 sm:mx-auto sm:max-w-2xl sm:px-6">
      ...
      {/* Receipt */}
      <div className="card-lift sheen bg-slate-900 border border-slate-800 rounded-2xl p-4 space-y-2.5 text-sm">
        <div className="flex justify-between">
          <span className="text-slate-400">Amount released</span>
          <span className="text-white font-bold text-emerald-300">{fmtMoney(p.released)}</span>
        </div>
        <div className="flex justify-between">
      ...
      <div className="flex gap-2">
        <button
          ...
          className="flex-1 py-2.5 rounded-xl bg-slate-900 border border-slate-800 text-slate-300 text-sm font-semibold flex items-center justify-center gap-1.5 transition-all duration-300 hover:border-slate-700 hover:text-white hover:scale-[1.02] active:scale-95"
        >
          <ArrowLeft className="w-4 h-4" /> Report
        </button>
        <button
          onClick={() => go("portfolio")}
          className="flex-1 py-2.5 rounded-xl bg-gradient-to-r from-purple-600 to-fuchsia-600 hover:from-purple-500 hover:to-fuchsia-500 text-white text-sm font-semibold shadow-lg shadow-purple-900/40 transition-all duration-300 hover:scale-[1.02] active:scale-[0.98]"
        >
          Back to portfolio
        </button>
      </div>
